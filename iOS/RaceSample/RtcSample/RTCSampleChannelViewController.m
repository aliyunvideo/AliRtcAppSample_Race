...
- (void)onBtnJoin:(id)sender {
    ...
    if (![self legalChannel]) {
        [self showAlertWithMessage:@"频道号只能是数字或字母,长度在3-12之间" handler:^(UIAlertAction * _Nonnull action) {
...
- (BOOL)legalChannel {
    NSString *pattern = @"^[0-9A-Za-z_]{3,12}$";
...
- (void)addSubviews {
    ...
    _tfChannel.backgroundColor = [UIColor grayColor];
    _tfChannel.clearButtonMode = UITextFieldViewModeAlways;
    _tfChannel.borderStyle = UITextBorderStyleRoundedRect;
    _tfChannel.placeholder = @"请输入频道号(3-12位)";
    _tfChannel.keyboardType  = UIKeyboardTypeNumbersAndPunctuation;
